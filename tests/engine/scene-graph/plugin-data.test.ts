...
describe('plugin data deduplication', () => {
  test('deduplicateNodeChangePluginData removes duplicate pluginData entries', () => {
    const entries = [
      { pluginID: 'nex-design', key: 'textDirection', value: 'RTL' },
      { pluginID: 'nex-design', key: 'textDirection', value: 'RTL' },
      { pluginID: 'nex-design', key: 'textDirection', value: 'RTL' }
    ]
  ...
  test('importNodeChanges with unique pluginData entries preserves all', () => {
    const entries = [
      { pluginID: 'nex-design', key: 'textDirection', value: 'RTL' },
      { pluginID: 'nex-design', key: 'layoutDirection', value: 'LTR' },
  ...
  test('importNodeChanges with single pluginData entry preserves it (zero-copy path)', () => {
    const entries = [{ pluginID: 'nex-design', key: 'textDirection', value: 'LTR' }]
...
describe('plugin data namespace collision guards', () => {
  test('getPluginDataKeys excludes encoded shared keys but allows private slash keys', () => {
    ...
    frame.setSharedPluginData('nex-design', 'bar', 'shared-value')

    const keys = frame.getPluginDataKeys()
    expect(keys).toContain('foo')
    expect(keys).toContain('foo/bar')
    expect(keys).not.toContain('nex-design/bar')
    expect(keys).not.toContain('bar')
  })

  test('getSharedPluginData does not read private nex-design plugin data', () => {
    const graph = new SceneGraph()
    const api = new FigmaAPI(graph)
    const frame = api.createFrame()
    ...
    // getSharedPluginData with the 'nex-design' namespace should NOT
    // return the private entry (which uses {pluginId: 'nex-design', key: 'textDirection'})
    const sharedValue = frame.getSharedPluginData('nex-design', 'textDirection')
  ...
  test('setSharedPluginData does not delete private nex-design plugin data', () => {
    const graph = new SceneGraph()
    const api = new FigmaAPI(graph)
    const frame = api.createFrame()
    ...
    expect(keys).not.toContain('nex-design/custom-key')
    expect(keys).not.toContain('custom-key')
  })

  test('getSharedPluginDataKeys does not leak private nex-design keys', () => {
    const graph = new SceneGraph()
    const api = new FigmaAPI(graph)
    const frame = api.createFrame()
    ...
    const sharedKeys = frame.getSharedPluginDataKeys('nex-design')
    expect(sharedKeys).not.toContain('textDirection')

    // Non-nex-design shared data is accessible
