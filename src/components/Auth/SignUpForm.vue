...
<template>
  <div class="space-y-5">
    <div class="space-y-1">
      ...
      <p class="text-xs text-zinc-400">Join designers building fast, vector-first interfaces</p>
    </div>
    ...
    <form @submit.prevent="handleSignUp" class="space-y-4 text-xs">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div
            ...
            :class="planId === 'plan-free' ? 'bg-rose-600/10 border-rose-500 text-white' : 'bg-zinc-950 border-white/5 text-zinc-400 hover:border-white/20'"
          >
          ...
          <div
            ...
            class="p-3 rounded-xl border cursor-pointer transition flex flex-col justify-between"
            :class="planId === 'plan-pro' ? 'bg-rose-600/10 border-rose-500 text-white' : 'bg-zinc-950 border-white/5 text-zinc-400 hover:border-white/20'"
          >
            <div class="font-bold text-xs flex items-center justify-between">
              Pro Designer
              <span class="text-[9px] bg-rose-600 text-white px-1.5 py-0.2 rounded uppercase">14d Trial</span>
            </div>
      ...
      <div class="pt-1">
        <label class="flex items-start gap-2 text-zinc-400 cursor-pointer">
          <input v-model="agreeTerms" type="checkbox" class="mt-0.5 rounded bg-zinc-950 border-white/10 text-rose-600 w-4 h-4 shrink-0" />
          <span>I agree to the <a href="#" class="text-rose-400 hover:underline">Terms of Service</a> and <a href="#" class="text-rose-400 hover:underline">Privacy Policy</a>.</span>
        </label>
      </div>
    ...
    <div class="text-center text-xs text-zinc-400 pt-2">
      ...
      <button @click="emit('switchView', 'signin')" class="text-rose-400 font-semibold hover:underline ml-1">
        Sign in
      </button>
    </div>
  </div>
