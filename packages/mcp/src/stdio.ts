...
const enableEval = process.env.NEXDESIGN_MCP_EVAL === '1'
const mcpRoot = process.env.NEXDESIGN_MCP_ROOT?.trim() || process.cwd()

const wsUrl = `ws://${wsHost}:${wsPort}`
const bridge = createStdioRpcBridge({
  wsUrl,
  onOpen: () => {
    process.stderr.write(`Connected to NexDesign app at ${wsUrl}\n`)
  },
  onMalformedMessage: () => {
...
const mcpServer = new McpServer({ name: 'nex-design', version: MCP_VERSION })
