import { buildFigmaClipboardHTML, buildNexDesignClipboardHTML } from '#core/clipboard'
...
export function createClipboardCopyActions(ctx: EditorContext) {
  async function writeCopyData(clipboardData: DataTransfer, selectedNodes: SceneNode[]) {
    ...
    clipboardData.setData('text/html', buildNexDesignClipboardHTML(selectedNodes, ctx.graph))
