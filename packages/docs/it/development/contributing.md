...
```
packages/
  core/              @nex-design/core — motore (zero dipendenze DOM)
    src/             Grafo scena, renderer, layout, codec, kiwi, tipi
  cli/               @nex-design/cli — CLI headless per operazioni .fig
    src/commands/    info, tree, find, export, eval, analyze
  mcp/               @nex-design/mcp — server MCP per strumenti IA
...
src/
  ...
  engine/            Shim di ri-esportazione da @nex-design/core
  kiwi/              Shim di ri-esportazione da @nex-design/core
...
Sviluppatori e agenti IA dovrebbero leggere `AGENTS.md` nella root del repo ([vedi su GitHub](https://github.com/nex-design/nex-design/blob/master/AGENTS.md)). Copre rendering, grafo scena, componenti e istanze, layout, UI, formato file, convenzioni Tauri e problemi noti.
