...
```sh
nex-design export design.fig                          # PNG (predefinito)
nex-design export design.fig -f jpg -s 2 -q 90       # JPG a 2×, qualità 90
nex-design export design.fig -f webp -s 3             # WEBP a 3×
nex-design export design.fig -f svg                   # SVG vettoriale
```
...
Supporta anche `--style nexdesign` per il formato JSX nativo (vedi [Renderer JSX](../jsx-renderer)).
...
```sh
nex-design export -f png    # screenshot del canvas corrente
```
