...
```sh
nex-design export design.fig                           # PNG (default)
nex-design export design.fig -f jpg -s 2 -q 90        # JPG at 2×, quality 90
nex-design export design.fig -f webp -s 3             # WEBP at 3×
nex-design export design.fig -f svg                   # SVG vector
nex-design export design.fig -f fig --page "Page 1"   # export one page as .fig
nex-design export design.fig -f fig --node 1:23        # export one node as .fig
```
...
Also supports `--style nexdesign` for the native JSX format (see [JSX Renderer](../jsx-renderer)).
