...
```sh
nex-design query design.fig "//TEXT"                    # All text nodes
nex-design query design.fig "//COMPONENT"               # All components
nex-design query design.fig "//INSTANCE"                # All instances
```

**By attributes:**

```sh
nex-design query design.fig "//FRAME[@width < 300]"                # Frames under 300px wide
nex-design query design.fig "//*[@cornerRadius > 0]"               # Rounded corners
nex-design query design.fig "//*[@visible = false]"                # Hidden nodes
nex-design query design.fig "//TEXT[@fontSize >= 24]"              # Large text
nex-design query design.fig "//*[@opacity < 1]"                    # Semi-transparent nodes
```

**By name and text content:**

```sh
nex-design query design.fig "//TEXT[contains(@name, 'Button')]"    # Name contains 'Button'
nex-design query design.fig "//TEXT[contains(@text, 'Hello')]"     # Text content contains 'Hello'
```
...
nex-design query design.fig "//FRAME/TEXT"               # Direct text children of frames
nex-design query design.fig "//COMPONENT_SET//INSTANCE"  # Instances inside component sets
```
...
nex-design eval -c "..."     # query the editor
```
