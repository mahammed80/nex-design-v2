---
title: Vue SDK
description: Создавайте редакторы на базе NexDesign с помощью headless-компосаблов и примитивов для Vue.
---

# Vue SDK

`@nex-design/vue` существует для того, чтобы NexDesign мог быть чем-то большим, чем самостоятельное дизайн-приложение.

Цель — сделать NexDesign набором инструментов, который можно встроить в другие продукты, внутренние инструменты и редакторы под конкретные задачи, а не только использовать в виде единого стандартного интерфейса.

Приложение NexDesign — одна из возможных компоновок этого набора. SDK — это то, с помощью чего вы создаёте другие.
...
- **Интеграция с фреймворком**: Vue-интеграция поверх `@nex-design/core`
